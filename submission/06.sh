#!/bin/bash

# Raw transaction hex
raw_tx=""

# Decode the transaction
decoded_tx=$(bitcoin-cli -regtest decoderawtransaction "$raw_tx")

# Extract and sum the output values
total_value=0
for value in $(echo "$decoded_tx" | grep -o '"value":[0-9.]*' | cut -d':' -f2); do
    # Convert BTC to satoshis (multiply by 100000000)
    satoshis=$(echo "$value * 100000000" | bc)
    total_value=$(echo "$total_value + $satoshis" | bc)
done

echo "Total output value in satoshis: $total_value"
